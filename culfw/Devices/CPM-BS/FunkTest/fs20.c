...
void fs20_init(void) {
     ...
     // outgoing signal
     SET_BIT( FS20_DDR, FS20_PINNUM );
     CLEAR_BIT( FS20_PORT, FS20_PINNUM );
...
void fs20_send( void ) {
     uint8_t data[4] = { 0x39, 0x4b, 0x00, 0x12 };
...
void fs20_send_adc( uint16_t adc ) {
     uint8_t data[4] = { 0x12, 0x34, 0x00, 0x00 };
