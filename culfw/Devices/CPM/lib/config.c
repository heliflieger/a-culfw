...
uint8_t
get_config_byte (uint16_t config)
{
...
void
get_config_n (uint16_t config, uint8_t * block, uint8_t size)
{
  for (uint8_t i = 0; i < size; i++)
    block[i] = eeprom_r1 (config + i);
  //return eeprom_rn(config, block, size);
}

void
set_config_byte (uint16_t config, uint8_t value)
{
...
void
set_config_n (uint16_t config, uint8_t * value, uint8_t size)
{
  for (uint8_t i = 0; i < size; i++)
    eeprom_w1 (config + i, value[i]);
