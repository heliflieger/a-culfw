...
/* enable radio bootloader */
#define RADIO_BOOT

uint8_t radio_active = 0;
 ...
 */
#define BLPORT		BOOTKEY_PORT
#define BLDDR		BOOTKEY_DDR
#define BLPIN		BOOTKEY_IN
#define BLPNUM		BOOTKEY_PIN
...
#ifdef BOOTKEY_REVERSE
	if (!radio_active && !bit_is_set(BLPIN,BLPNUM)) {
#else
	if (!radio_active && bit_is_set(BLPIN,BLPNUM)) {
#endif
